...
# class StartWindow(QWidget):
    ...
    # def __init__(self):
        # # call QWidget constructor
        # super().__init__()
        # self.ui = StartWindowForm()
        # self.ui.setupUi(self)
        
        # # set control_bt callback clicked  function
        # self.ui.pushButton.clicked.connect(self.switchScreens)
...
class Visualizer:
    ...
    def draw_detection_roi(self, frame, roi, identity):
        # Here's how to pull out the name of the person identified
        ...
        if identity.id != FaceIdentifier.UNKNOWN_ID:
            text += ' %.2f%%' % (100.0 * (1 - identity.distance))  # Here's how to access the confidence percentage
    ...
    def document_detections(self, frame, detections):

        with open('faces_detected.csv', mode='w') as detections_file:
            detections_writer = csv.writer(detections_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            for roi, landmarks, identity in zip(*detections):
                nameText = self.frame_processor.face_identifier.get_identity_label(identity.id)

                detectionConfidence = 100.0 * (1 - identity.distance)
                detections_writer.writerow([nameText, detectionConfidence])



    def process(self, input_stream, output_stream, mainWindow):
        ...
        while input_stream.isOpened():
            ...
            for roi, landmarks, identity in zip(*detections):
                detectionConfidence = 100.0 * (1 - identity.distance)
                if detectionConfidence > 87.0:
                    self.display = False
                else:
                    self.display = True
            
            self.document_detections(frame, detections) # Brandon 2/15/2020
            if len(self.found) > 0:
                ordered_list=list(self.found)
                name = ordered_list[0]
                latest = self.found[name]
                for item in ordered_list:
                    if self.found[item] > latest:
                        name = item
                        latest = self.found[name]
                # ordered_list=sorted(ordered_list,key=ordered_list.get, reverse=True)
                #mainWindow.welcome_back_label.setText(_translate("Form", "Welcome back, "+str(self.found[0])+"!"))
                if name != 'Unknown':
                    mainWindow.ui.welcome_back_label.setText("<html><head/><body><p><span style=\" color:#ffffff;\">Welcome Back, "+name+"!</span></p></body></html>")
                # label = "Welcome Back, {}".format(list(self.found)[0])
                # mainWindow.ui.welcome_back_label.setText(label)
            ...
            if self.display:
                ...
                step = channel * width
                # create QImage from RGB frame    B&D 3/7/2020
                qImg = QImage(frame.data, width, height, step, QImage.Format_RGB888)
                # show frame in img_label B&D 3/7/2020
    ...
    def run(self, args, mainWindow):
        ...
        self.process(input_stream, output_stream, mainWindow)
...
def main():
    ...
    #print(vars(args))
    #{'input': '0', 'output': '', 'no_show': False, 'timelapse': False, 'crop_width': 0, 'crop_height': 0, 'fg': 'C:\\Users\\kinet\\Documents\\Kinetic Playground\\face pics', 'run_detector': False, 'm_fd': 'C:\\Program Files (x86)\\IntelSWTools\\openvino_2019.3.379\\deployment_tools\\tools\\model_downloader\\intel\\face-detection-retail-0004\\INT8\\face-detection-retail-0004.xml', 'm_lm': 'C:\\Program Files (x86)\\IntelSWTools\\openvino_2019.3.379\\deployment_tools\\tools\\model_downloader\\intel\\landmarks-regression-retail-0009\\FP16\\landmarks-regression-retail-0009.xml', 'm_reid': 'C:\\Program Files (x86)\\IntelSWTools\\openvino_2019.3.379\\deployment_tools\\tools\\model_downloader\\intel\\face-reidentification-retail-0095\\FP16\\face-reidentification-retail-0095.xml', 'd_fd': 'CPU', 'd_lm': 'CPU', 'd_reid': 'CPU', 'cpu_lib': 'C:\\Program Files (x86)\\IntelSWTools\\openvino_2019.3.379\\deployment_tools\\inference_engine\\bin\\intel64\\Release\\cpu_extension_avx2.dll', 'gpu_lib': '', 'verbose': False, 'perf_stats': False, 't_fd': 0.6, 't_id': 0.3, 'exp_r_fd': 1.15, 'allow_grow': False}
    myArgs = argparse.ArgumentParser(description='The arguments')
    myArgs.add_argument('-m_fd', action="store")
    myArgs.add_argument('--output', action="store")
    myArgs.add_argument('--no_show', action="store")
    
    myArgs.parse_args(['-m_fd', 'C:\Program Files (x86)\IntelSWTools\openvino_2019.3.379\deployment_tools\tools\model_downloader\intel\face-detection-retail-0004\INT8\face-detection-retail-0004.xml'])
    print(vars(myArgs))
    ...
    visualizer.run(args, mainWindow)
